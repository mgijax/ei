...
--
	  ...
	  set : string := "";
		...
		currentRecordKey + "," +
		newKey + ",(select max(sequencenum) + 1 from MGI_Organism_MGIType)," +
		global_userKey + "," + global_userKey + END_VALUE;
            elsif (editMode = TBL_ROW_MODIFY) then
...
--
	...
	Search does
	  ...
	  QueryNoInterrupt.select := "select s._Organism_key, s.commonName || ' (' || s.latinName || ')' as commonName\n" + from + "\n" + where + "\norder by commonName\n";
	  QueryNoInterrupt.table := MGI_ORGANISM;
