...
-- Database Tables Affected:	ALL_Allele, ALL_Molecular_Note, ALL_Note, ALL_Synonym
...
--
	...
	Modify does
	  ...
	  if (top->mgiCitation1->ObjectID->text.modified) then
	    set := set + "_Molecular_Refs_key = " + mgi_DBprkey(top->mgiCitation1->ObjectID->text.value) + ",";
	  end if;
...
--
	...
	PrepareSearch does
	  ...
	  from_molreference : boolean := false;
	  from_mutation     : boolean := false;
	  from_synonym      : boolean := false;
	  from_molnote      : boolean := false;
	  from_allnote      : boolean := false;
	  ...
	  if (value.length > 0 and value != "NULL") then
	    ...
	    where := where + "\nand a.name like " + mgi_DBprstr(top->Name->text.value);
          ...
          if (top->ReviewedMenu.menuHistory.searchValue != "%") then
            where := where + "\nand a.reviewed = " + top->ReviewedMenu.menuHistory.searchValue;
          end if;
	  ...
	  if (top->mgiCitation->ObjectID->text.value.length > 0) then
	    where := where + "\nand a._Refs_key = " + top->mgiCitation->ObjectID->text.value;
	  elsif (top->mgiCitation->Jnum->text.value.length > 0) then
	    where := where + "\nand r.jnum = " + top->mgiCitation->Jnum->text.value;
	    from_reference := true;
	  elsif (top->mgiCitation->Citation->text.value.length > 0) then
	    where := where + "\nand r.short_citation like " + mgi_DBprstr(top->mgiCitation->Citation->text.value);
	    from_reference := true;
	  end if;

	  if (top->mgiCitation1->ObjectID->text.value.length > 0) then
	    where := where + "\nand a._Molecular_Refs_key = " + top->mgiCitation1->ObjectID->text.value;
	  elsif (top->mgiCitation1->Jnum->text.value.length > 0) then
	    where := where + "\nand mr.jnum = " + top->mgiCitation1->Jnum->text.value;
	    from_molreference := true;
	  elsif (top->mgiCitation1->Citation->text.value.length > 0) then
	    where := where + "\nand mr.short_citation like " + mgi_DBprstr(top->mgiCitation1->Citation->text.value);
	    from_molreference := true;
	  end if;
	  ...
	  if (value.length > 0 and value != "NULL") then
	    ...
	    where := where + "\nand mn.note like " + mgi_DBprstr(top->MolecularNote->text.value);
	    from_molnote := true;
	  end if;
	  ...
	  if (from_reference) then
	    from := from + "," + mgi_DBtable(ALL_REFS_VIEW) + " r";
	    where := where + "\nand a." + mgi_DBkey(ALL_ALLELE) + " = r." + mgi_DBkey(ALL_ALLELE);
	  end if;

	  if (from_molreference) then
	    from := from + "," + mgi_DBtable(ALL_MOLREFS_VIEW) + " mr";
	    where := where + "\nand a." + mgi_DBkey(ALL_ALLELE) + " = mr." + mgi_DBkey(ALL_ALLELE);
	  end if;
	  ...
	  if (from_molnote) then
	    from := from + "," + mgi_DBtable(ALL_MOLECULAR_NOTE) + " mn";
	    where := where + "\nand a." + mgi_DBkey(ALL_ALLELE) + " = mn." + mgi_DBkey(ALL_ALLELE);
	  end if;

	  if (from_allnote) then
...
--
	  ...
	  cmd := "select * from " + mgi_DBtable(ALL_ALLELE_VIEW) +
		 ...
		 " where " + mgi_DBkey(ALL_ALLELE) + " = " + currentRecordKey + "\n" +
		 "select note from " + mgi_DBtable(ALL_MOLECULAR_NOTE) +
		 " where " + mgi_DBkey(ALL_ALLELE) + " = " + currentRecordKey +
		 " order by sequenceNum\n" +
		 "select note from " + mgi_DBtable(ALL_NOTE) +
		 " where " + mgi_DBkey(ALL_ALLELE) + " = " + currentRecordKey +
		 " order by sequenceNum\n";
	  ...
	  while (dbresults(dbproc) != NO_MORE_RESULTS) do
	    ...
	    while (dbnextrow(dbproc) != NO_MORE_ROWS) do
	      if (results = 1) then
	        ...
	        top->Symbol->text.value       := mgi_getstr(dbproc, 10);
	        top->Name->text.value         := mgi_getstr(dbproc, 11);
	        top->CreationDate->text.value := mgi_getstr(dbproc, 12);
	        top->ModifiedDate->text.value := mgi_getstr(dbproc, 13);
		top->ModifiedBy->text.value   := mgi_getstr(dbproc, 9);

		top->mgiMarker->ObjectID->text.value := mgi_getstr(dbproc, 4);
		top->mgiMarker->Marker->text.value := mgi_getstr(dbproc, 14);

		top->EditForm->Strain->StrainID->text.value := mgi_getstr(dbproc, 5);
		top->EditForm->Strain->Verify->text.value := mgi_getstr(dbproc, 15);
                ...
                SetOption.source_widget := top->ReviewedMenu;
