...
--
-- Events which handle processing creating/cloning widgets for XRT/table
--

--
-- CreateTableMenuWidget
--
-- Creating/cloning widgets for which XRT/table does not know the widget's class.
--
-- The event must be assigned to the XmNxrtTblCreateWidgetCallback for the 
-- appropriate XRT/table widget.
--

	CreateTableMenuWidget does
	  table : widget := CreateTableMenuWidget.source_widget;
	  menu : string := CreateTableMenuWidget.menu;
	  parent : widget;
	  newWidget : widget;

	  if (CreateTableMenuWidget.reason != TBL_REASON_CREATE_WIDGET_BEGIN) then
	    return;
	  end if;

	  parent := mgi_tblGetCallbackParent(CreateTableMenuWidget.callback_struct);

	  -- Create the menu widget
	 newWidget := create widget(menu, nil, parent);
	 newWidget.managed := false;

	 -- Initialize the menu widget

	 if (newWidget.class_name = COMBO_CLASS) then
           InitComboMenu.option := newWidget;
	   send(InitComboMenu, 0);
	 else
  	   if (CreateTableMenuWidget.row = 0) then
             InitOptionMenu.option := newWidget;
  	     send(InitOptionMenu, 0);
	   end if;
  	 end if;

	 CreateTableMenuWidget.new_widget := newWidget;
	end does;

--
-- DisplayTableMenuWidget
--
-- The event must be assigned to the XmNxrtTblDisplayWidgetCallback for the 
-- appropriate XRT/table widget.
--

	DisplayTableMenuWidget does
	  table : widget := DisplayTableMenuWidget.source_widget;
	  row : integer := DisplayTableMenuWidget.row;
	  column : integer := DisplayTableMenuWidget.column;

	  menu : widget := XrtTblGetWidgetByRowCol(table, row, column);

	  if (menu = nil) then
	    return;
	  end if;

	  history : widget := menu.menuHistory;
	  (void) mgi_writeLog(history.defaultValue + "\n");
	  (void) mgi_writeLog(history.name + "\n");
...
-- SelectTableMenu
--
-- The event must be assigned to the XmNxrtTblEnterCellCallback for the 
-- appropriate XRT/table widget.
--

	SelectTableMenu does
--	  table : widget := SelectTableMenu.source_widget;
	  reason : integer := SelectTableMenu.reason;
--	  row : integer := SelectTableMenu.row;
--	  column : integer := SelectTableMenu.column;

--	  tu_printf("Widget:  %d\n", SelectTableMenu.source_widget.name);
--	  tu_printf("Reason:  %d\n\n", reason);
	  return;

	  if (SelectTableMenu.reason != TBL_REASON_ENTER_CELL_END) then
	    return;
	  end if;
...
--	  SetOption.source_widget := menu;
--	  SetOption.value := menu.menuHistory.defaultValue;
--	  send(SetOption, 0);

	end does;

--
-- InsertTableLabelWidget
--	table : widget;          	the table widget
--	template : string;		the template of the widget to add
--	column : integer;		the column at which to add the widget
--
-- Insert widget into XRT/table label using xrtTblWidgetSeries resource
--
--	InsertTableLabelWidget.table := table;
--	InsertTableLabelWidget.template := "XmPushButton";
--	InsertTableLabelWidget.column := 2;
--	send(InsertTableLabelWidget, 0);
--

	InsertTableLabelWidget does
	  table : widget := InsertTableLabelWidget.table;
	  template : string := InsertTableLabelWidget.template;
	  column : integer := InsertTableLabelWidget.column;

	  table.xrtTblWidgetSeries := 
		"(" + (string) XRTTBL_LABEL + " " + (string) column + " " + template + ")";
	end does;
