...
char *mgi_DBinsert(int table, char *keyName)
{
  ...
  switch (table)
  {
    ...
    case MGI_SYNONYM:
	    ...
	    break;
    case MGI_SYNONYMTYPE:
            sprintf(buf, "insert into %s (%s, _MGIType_key, _Organism_key, synonymType, _CreatedBy_key, _ModifiedBy_key)", mgi_DBtable(table), mgi_DBkey(table));
	    break;
    case MGI_TRANSLATION:
    ...
    case MRK_OFFSET:
	    sprintf(buf, "insert into %s (%s, source, offset)",
 	    ...
 	    break;
    case NOM_MARKER:
            sprintf(buf, "insert into %s (%s, _Marker_Type_key, _NomenStatus_key, _Marker_Event_key, _Marker_EventReason_key, _CurationState_key, symbol, name, chromosome, humanSymbol, statusNote, _CreatedBy_key, _ModifiedBy_key)",
