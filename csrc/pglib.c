...
* mgi_dbexit		done
*
* mgi_dbexec		in progress
* mgi_dbresults		in progress
* mgi_dbnextrow		in progress
*
* mgi_execute_search
* mgi_getstr		1/2 done
* mgi_citation
...
int mgi_dbinit(char *user, char *pwd)
{
   ...
   * but want to use the env variable MGD_DBNAME and the user-entered user/password
   */

  /*sprintf(conninfo, "dbname = %s user = %s password = %", getenv("MGD_DBNAME"), user, pwd); */

  conninfo = "dbname = lec user = mgd_dbo password = db0dev";
  printf("mgi_dbinit: %s\n", conninfo);
...
*
* submits a command to the server and wait for the result
*
...
int mgi_dbresults(PGconn *conn)
{
  ...
  */
  ...
  /*
  *
  * PGRES_TUPLES_OK: sucessful completion of a command returning data
  * set global maxRow
