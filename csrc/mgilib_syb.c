...
char *mgi_DBinsert(int table, char *keyName)
{
  ...
  switch (table)
  {
    ...
    case GXD_ASSAYTYPE:
	    ...
	    break;
    case GXD_ANTIBODY:
            sprintf(buf, "insert %s (%s, _AntibodyClass_key, _AntibodyType_key, _Organism_key, _Antigen_key, antibodyName, antibodyNote, _CreatedBy_key, _ModifiedBy_key)", 
		mgi_DBtable(table), mgi_DBkey(table));
	    break;
    case GXD_ANTIBODYMARKER:
    ...
    case MRK_ANCHOR:
	    ...
	    break;
    case MRK_CHROMOSOME:
            sprintf(buf, "insert %s (%s, _Organism_key, chromosome, sequenceNum, _CreatedBy_key, _ModifiedBy_key)", mgi_DBtable(table), mgi_DBkey(table));
	    break;
    case MRK_CLASSES:
    ...
    case SEQ_ALLELE_ASSOC:
	    ...
	    break;
    case STRAIN:
            sprintf(buf, "insert %s (%s, _Species_key, _StrainType_key, strain, standard, private, geneticBackground, _CreatedBy_key, _ModifiedBy_key)", mgi_DBtable(table), mgi_DBkey(table));
	    break;
    case TISSUE:
    ...
    case VOC_TEXT:
	    ...
	    break;
    case VOC_ANNOTHEADER:
            sprintf(buf, "insert %s (%s, _AnnotType_key, _Object_key, _Term_key, sequenceNum, isNormal,_CreatedBy_key, _ModifiedBy_key, _ApprovedBy_key, approval_date)", mgi_DBtable(table), mgi_DBkey(table));
	    break;
    case VOC_ANNOTTYPE:
