#!/bin/csh
 
#
# Generates the QC report for the Alternative Transcripts Project
#
# Probes of mouse source which encode more than one gene and which contain
# a note pertaining to alternative transcripts.
#
 
setenv DSQUERY $1
setenv MGD $2
 
header.sh $0
 
isql -S$DSQUERY -Umgd_public -Pmgdpub -w200 <<END >> $HOME/mgireport/$0.rpt
 
use $MGD
go
 
set nocount on
go

select distinct      mm.symbol, pb._Probe_key, probe_name = (convert(varchar (20), pb.name))
into                 #csnr
from                 PRB_Reference pr, PRB_Source ps, PRB_Probe pb, PRB_Notes pn,
                     MRK_Marker mm, PRB_Marker pm
where                pn.note like "%alternat%"
   and               ps.species = "mouse, laboratory"
   and               pm.relationship = "E"
   and               pn._Probe_key = pb._Probe_key
   and               pb._Probe_key = pr._Probe_key
   and               pb._Source_key = ps._Source_key
   and               mm._Marker_key = pm._Marker_key
   and               pm._Probe_key = pb._Probe_key
go
 
set nocount off
go

print ""
print "QC report for Alterntative Transcripts public report (PRB_AltTranscripts.rpt)."
print ""
 
select distinct      c.probe_name, c._Probe_key, count(c.symbol)
from                 #csnr c
group by             c._Probe_key
having               count(c.symbol) > 1
go 

quit

END

