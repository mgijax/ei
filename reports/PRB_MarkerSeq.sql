#!/bin/csh
 
#
# TR 1270
#
# 
# 1.List all I.M.A.G.E. clones which contain a Seq ID which cannot be 
#   found in the set of Seq IDs for the encoding marker. 
#
# 2.List all markers which contain a Seq ID which cannot be found in 
#   the set of Seq IDs for the I.M.A.G.E. clone which it encodes. 
# 
# 3.List all I.M.A.G.E. clones which contain a Seq ID which can be 
#   found attached to a Marker which does not have an encodes relationship 
#   with the clone. 
#
# Notes:
#	- all public reports require a header and trailer
#	- all private reports require a header
#


setenv DSQUERY $1
setenv MGD $2

header.sh $0

isql -S$DSQUERY -Umgd_public -Pmgdpub -w200 <<END >> $HOME/mgireport/$0.rpt

set nocount on
go

use $MGD
go

select p._Probe_key, p.name, pm._Marker_key, caccID = pa.accID, maccID = ma.accID
into #clone
from PRB_Probe p, PRB_Marker pm, PRB_Acc_View pa, MRK_Acc_View ma
where p.name = 'I.M.A.G.E. clone'
and p._Probe_key = pm._Probe_key
and pm.relationship = "E"
and p._Probe_key = pa._Object_key
and pa.prefixPart = "MGI:"
and pa.preferred = 1
and pm._Marker_key = ma._Object_key
and ma.prefixPart = "MGI:"
and ma.preferred = 1
go

set nocount off
go

print ""
print "I.M.A.G.E. Clones with Seq IDs which do not exist for encoding Markers"
print ""

select distinct c.caccID "Clone", c.maccID "Marker", a.accID
from #clone c, PRB_AccRef_View a
where c._Probe_key = a._Object_key
and a._LogicalDB_key = 9
and not exists (select 1 from MRK_AccRef_View ma
where c._Marker_key = ma._Object_key
and ma.accID = a.accID)
order by c.caccID
go

print ""
print "Markers with Seq IDs which do not exist for Clones to which the Marker encodes"
print ""

select distinct c.maccID "Marker", c.caccID "Clone", a.accID
from #clone c, MRK_AccRef_View a
where c._Marker_key = a._Object_key
and a._LogicalDB_key = 9
and not exists (select 1 from PRB_AccRef_View ma
where c._Probe_key = ma._Object_key
and ma.accID = a.accID)
order by c.maccID
go

set nocount on
go

drop table #clone
go

select p._Probe_key, pm._Marker_key, a.accID
into #clone
from PRB_Probe p, PRB_Marker pm, ACC_Accession a, ACC_Accession ma
where p.name = 'I.M.A.G.E. clone'
and p._Probe_key = pm._Probe_key
and pm.relationship != "E"
and p._Probe_key = a._Object_key
and a._MGIType_key = 3
and a._LogicalDB_key = 9
and pm._Marker_key = ma._Object_key
and ma._MGIType_key = 2
and ma._LogicalDB_key = 9
and ma.accID = a.accID
go

set nocount off
go

print ""
print "I.M.A.G.E. Clones with Seq IDs which exist for non-encoding Markers"
print ""

select distinct ca.accID "Clone", ma.accID "Marker", c.accID
from #clone c, PRB_Acc_View ca, MRK_Acc_VIew ma
where c._Probe_key = ca._Object_key
and ca.prefixPart= "MGI:"
and ca.preferred = 1
and c._Marker_key = ma._Object_key
and ma.prefixPart= "MGI:"
and ma.preferred = 1
order by ca.accID
go

quit

END

cat trailer >> $HOME/mgireport/$0.rpt

