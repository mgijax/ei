...
def parse_alias(alias):
	...
	alias_string = alias_string + mgi_utils.prvalue(alias['alias'])
	write(reportlib.format_line(mgi_utils.prvalue(alias['alias'])))

def parse_marker(marker):
	write(TAB + string.ljust(mgi_utils.prvalue(marker['symbol']), 20))
	write(string.ljust(mgi_utils.prvalue(marker['chromosome']), 20))
	write(string.ljust(mgi_utils.prvalue(marker['relationship']), 20) + crt(1))

def parse_clone(probe):
	write(TAB + string.ljust('Parent Clone:  ', 15) + \
	      mgi_utils.prvalue(probe['parentClone']) + \
...
def parse_source(probe):
	...
	write(string.rjust('Strain:  ', 15) + \
		...
		string.ljust(mgi_utils.prvalue(probe['tissue']), 20))
       	write(string.rjust('Cell Line:  ', 15) + \
		string.ljust(mgi_utils.prvalue(probe['cellLine']), 20) + crt(1))
       	write(string.rjust('Tissue Treatment:  ', 20) + \
		string.ljust(mgi_utils.prvalue(probe['description']), 30) + crt(1))
       	write(TAB + string.ljust('Age:', 15) + \
		string.ljust(mgi_utils.prvalue(probe['age']), 20))
       	write(string.rjust('Sex:  ', 15) + \
		string.ljust(mgi_utils.prvalue(probe['sex']), 20) + crt(1))

def parse_note(note):
	write(reportlib.format_line(mgi_utils.prvalue(note['note'])))
...
def parse_alleles(alleles):
	...
	if (present_rflv_key != alleles['_RFLV_key']):
                write(string.ljust(mgi_utils.prvalue(alleles['endonuclease']), 15))
		write(string.ljust(mgi_utils.prvalue(alleles['symbol']), 15))
           	present_rflv_key = alleles['_RFLV_key']
        else:
                write(string.ljust(' ', 30))

	if (present_allele_key != alleles['_Allele_key']):	
        	write(string.ljust(mgi_utils.prvalue(alleles['allele']), 10))
		write(string.ljust(mgi_utils.prvalue(alleles['fragments']), 30))
		present_allele_key = alleles['_Allele_key']
	else:
                write(string.ljust(' ', 40))

 	write(mgi_utils.prvalue(alleles['strain']) + crt(1))
...
def parse_probe(probe):
	...
	header = header + string.ljust('Name:  ', 10) + mgi_utils.prvalue(probe['name']) + 4*TAB
	...
	write(string.rjust('Sequence Type:  ', 15) + \
		string.ljust(mgi_utils.prvalue(probe['DNAtype']), 20) + crt(1))
	write(reportlib.format_line('Region Covered:  ' + \
        mgi_utils.prvalue(probe['regionCovered']) + \
		mgi_utils.prvalue(probe['regionCovered2'])) + crt(2))
	...
	db.sql(cmd, parse_marker)
	write(crt(2))

	if probe['DNAtype'] == 'primer':
        	write('Product size:  ' + \
			mgi_utils.prvalue(probe['productSize']) + crt(1))
        	write('Sequence 1  :  ' + \
			mgi_utils.prvalue(probe['primer1sequence']) + crt(1))
        	write('Sequence 2  :  ' + \
			mgi_utils.prvalue(probe['primer2sequence']) + crt(1))
        	write('Repeat unit :  ' + \
			reportlib.format_line(mgi_utils.prvalue(probe['repeatUnit'])) + crt(1))
	else:
		if probe['derivedFrom'] is not None:
			...
			db.sql(cmd, parse_clone)

		cmd = 'select * from PRB_Source_View ' + \
		      'where _Source_key = %d' % probe['_Source_key']
		db.sql(cmd, parse_source)

       		write(TAB + string.ljust('Vector Type:  ', 15) + \
			string.ljust(mgi_utils.prvalue(probe['vectorType']), 20))
       		write(string.rjust('Insert site:  ', 15) + \
			string.ljust(mgi_utils.prvalue(probe['insertSite']), 20))
		write(crt(1) + TAB)

       	write(string.ljust('Insert size:  ', 15) + \
		string.ljust(mgi_utils.prvalue(probe['insertSize']), 20) + crt(2))

	write('Comments:  ' + crt(1))
	cmd = 'select note from PRB_Notes where _Probe_key = %d' % probe['_Probe_key']
	db.sql(cmd, parse_note)
	write(crt(1) + 80 * '_' + crt(2))

	cmd = 'select * from PRB_Reference_View where _Probe_key = %d' % probe['_Probe_key']
	db.sql(cmd, parse_ref)

def parse_ref(ref):
	global probeName

	write(string.ljust('Name:  ', 10) + mgi_utils.prvalue(probeName) + crt(1))
	write(string.ljust('Ref:  ', 10) + 'J' + mgi_utils.prvalue(ref['jnum']) + \
	      SPACE + mgi_utils.prvalue(ref['short_citation']) + crt(1))
	write(string.ljust('Holder:  ', 10) + mgi_utils.prvalue(ref['holder']) + crt(1))
